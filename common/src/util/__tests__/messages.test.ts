...
describe('convertCbToModelMessages', () => {
  ...
  describe('tool message conversion', () => {
    ...
    it('should convert tool messages with media output', () => {
      ...
      // Media must stay inside a `tool-result` so the assistant's `tool-call`
      // has a matching pair; otherwise the model retries the tool call in a
      // loop. The AI SDK's `output.type === 'content'` channel carries the
      // image through to the provider as an image block in tool_result.
      expect(result).toEqual([
        expect.objectContaining({
          role: 'tool',
          content: [
  ...
  describe('cache control', () => {
    ...
    test('should always cache system message even without USER_PROMPT/STEP_PROMPT tags (T0.2)', () => {
      // Long history with no anchor tags — previously, the walk-back from
      // USER_PROMPT/STEP_PROMPT was the only path that reached index 0, so the
      // system message went uncached in this shape. T0.2 makes system caching
      // unconditional when includeCacheControl is true.
      const messages: Message[] = [
        systemMessage('Stable system prompt'),
        userMessage('Untagged 1'),
        assistantMessage('Response 1'),
        userMessage('Untagged 2'),
      ...
      expect(systemMsg.providerOptions?.anthropic?.cache_control).toEqual({
        type: 'ephemeral',
      })
      expect(systemMsg.providerOptions?.openrouter?.cache_control).toEqual({
        type: 'ephemeral',
      })
    })

    test('should cache KNOWLEDGE_FILES-tagged segment as its own breakpoint (T0.3)', () => {
      // Mirrors the shape produced by buildSystemMessagesWithKnowledgeCache:
      // [stable system, knowledge-files system, post-knowledge system, user...]
      const messages: Message[] = [
        systemMessage('Stable system'),
        systemMessage({
          content: 'Knowledge files content',
          tags: ['KNOWLEDGE_FILES'],
        }),
        systemMessage('Post-knowledge system'),
      ...
      // Both the stable-system (index 0) and knowledge-files (index 1) must
      // carry cache_control — they are independent breakpoints.
      const stableSystem = result[0] as {
        providerOptions?: Record<string, ProviderWithCacheControl>
      }
      const knowledgeSystem = result[1] as {
        providerOptions?: Record<string, ProviderWithCacheControl>
      }
      expect(stableSystem.providerOptions?.anthropic?.cache_control).toEqual({
        type: 'ephemeral',
      })
      expect(knowledgeSystem.providerOptions?.anthropic?.cache_control).toEqual(
        {
          type: 'ephemeral',
        },
      )
    ...
    test('should cache PROJECT_MEMORY-tagged segment as its own breakpoint (T0.4)', () => {
      // Mirrors the shape produced by buildSystemMessagesWithKnowledgeCache:
      // [stable system, knowledge-files system, project-memory system, post-memory system, user...]
      const messages: Message[] = [
        systemMessage('Stable system'),
        systemMessage({
          content: 'Knowledge files content',
        ...
        systemMessage('Volatile tail (git changes, system info)'),
        userMessage({ content: 'User prompt', tags: ['USER_PROMPT'] }),
      ]
      ...
      const memorySystem = result[2] as {
        providerOptions?: Record<string, ProviderWithCacheControl>
      }
      expect(stableSystem.providerOptions?.anthropic?.cache_control).toEqual({
        type: 'ephemeral',
      ...
      expect(memorySystem.providerOptions?.anthropic?.cache_control).toEqual({
        type: 'ephemeral',
      })
    })

    test('message-level cache breakpoints are capped at 3 to leave room for tools-schema slot (T0.4)', () => {
      // With MAX_CACHE_BREAKPOINTS=3, the stable content (system, knowledge,
      // memory) uses all 3 message-level slots; no conversation breakpoints are
      // added. The 4th Anthropic slot is reserved for the tools-schema
      // breakpoint applied in loopAgentSteps.
      const messages: Message[] = [
        systemMessage('System'),
        systemMessage({ content: 'Knowledge', tags: ['KNOWLEDGE_FILES'] }),
        systemMessage({ content: 'Memory', tags: ['PROJECT_MEMORY'] }),
        userMessage('Pre-history A'),
        assistantMessage('A response'),
        userMessage({ content: 'Step', tags: ['STEP_PROMPT'] }),
        userMessage({ content: 'User prompt', tags: ['USER_PROMPT'] }),
        assistantMessage({
      ...
      const countBreakpoints = (msg: ModelMessage): number => {
        let count = 0
        if (
          (msg.providerOptions as Record<string, ProviderWithCacheControl> | undefined)
        ...
        if (typeof msg.content !== 'string' && Array.isArray(msg.content)) {
          for (const part of msg.content) {
            const partOptions = (part as { providerOptions?: Record<string, ProviderWithCacheControl> })
              .providerOptions
            if (partOptions?.anthropic?.cache_control) {
              count++
            }
          }
        }
      ...
      const totalBreakpoints = result.reduce(
        (sum, msg) => sum + countBreakpoints(msg),
        0,
      )
      // Message-level breakpoints must not exceed 3 so the 4th Anthropic slot
      // remains available for tools schema (applied outside convertCbToModelMessages).
      expect(totalBreakpoints).toBeLessThanOrEqual(3)
    })

    test('should not exceed 4 cache breakpoints across system + anchors (T0.2)', () => {
      // System + 4 anchors that all walk back to distinct indices would be 5
      // breakpoints — over Anthropic's limit. The cap-tracking must keep total
      // distinct cached indices ≤ 4.
      const messages: Message[] = [
        systemMessage('System'),
        userMessage('Pre-history A'),
        assistantMessage('A response'),
        userMessage('Pre-history B'),
        userMessage({ content: 'Step', tags: ['STEP_PROMPT'] }),
        userMessage({ content: 'User prompt', tags: ['USER_PROMPT'] }),
        assistantMessage({
          content: 'Last response',
      ...
      // System is always one of the breakpoints
      const systemMsg = result[0] as {
        role: 'system'
        providerOptions?: Record<string, ProviderWithCacheControl>
      }
