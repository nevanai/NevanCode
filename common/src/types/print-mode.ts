...
// Live context-window usage for the main agent, emitted once per step so the
// CLI bar can show the REAL model context (token count + the operational
// window the pruner compacts against) as it changes mid-run — not just a
// transcript estimate between turns.
export const printModeContextUsageSchema = z.object({
  type: z.literal('context_usage'),
  contextTokenCount: z.number(),
  maxContextTokens: z.number(),
  agentId: z.string().optional(),
})
export type PrintModeContextUsage = z.infer<
  typeof printModeContextUsageSchema
>
