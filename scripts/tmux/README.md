...
### Local Dev OpenAI Smoke

Use the dedicated harness when you want to verify the `bun start-cli` runtime,
the active config directory, and the `/model` persistence path together:

```bash
./scripts/tmux/smoke-dev-runtime.sh
./scripts/tmux/smoke-dev-runtime.sh gpt-5.5 "hi"
```

The harness starts `bun start-cli`, captures the startup header, runs
`/model`, checks `~/.config/manicode-dev/settings.json`, sends a prompt, and
reports whether it saw a reconnect/blocking message or a lingering
`connecting...` state.
