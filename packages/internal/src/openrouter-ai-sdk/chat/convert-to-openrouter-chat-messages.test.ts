...
describe('tool-result images (computer-use / browser screenshots)', () => {
  const b64 = 'QUJDREVG' // "ABCDEF"

  it('relocates a screenshot into a following user message as image_url', () => {
    const result = convertToOpenRouterChatMessages([
      {
        role: 'tool',
        content: [
  ...
  it('never leaks raw base64 into tool message content', () => {
  ...
  it('keeps json tool results stringified (no regression)', () => {
    const result = convertToOpenRouterChatMessages([
      {
        role: 'tool',
        content: [
