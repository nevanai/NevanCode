...
describe('tool-result images (computer-use / browser screenshots)', () => {
  const b64 = Buffer.from([0, 1, 2, 3, 4, 5]).toString('base64')

  it('relocates a screenshot image into a following user message as image_url', () => {
    const result = convertToOpenAICompatibleChatMessages([
      {
        role: 'tool',
        content: [
          {
            ...
            output: {
              ...
              value: [
                ...
                { type: 'media', data: b64, mediaType: 'image/jpeg' },
              ],
            },
          },
        ],
  ...
  it('never emits the raw base64 inside the tool message content', () => {
        ...
        content: [
          {
            ...
            output: {
              type: 'content',
              value: [{ type: 'media', data: bigB64, mediaType: 'image/png' }],
            },
          },
        ],
      },
    ...
    expect(toolMsg.content as string).not.toContain(bigB64)
    const userMsg = result.find((m) => m.role === 'user')!
    expect(JSON.stringify(userMsg.content)).toContain(bigB64)
  })

  it('keeps tool results before the relocated user image for parallel calls (pairing intact)', () => {
    const result = convertToOpenAICompatibleChatMessages([
      {
        role: 'tool',
        content: [
          {
            ...
            output: {
              type: 'content',
              value: [{ type: 'media', data: b64, mediaType: 'image/jpeg' }],
            },
          },
          {
            type: 'tool-result',
    ...
    expect(roles).toEqual(['tool', 'tool', 'user'])
    expect((result[0] as any).tool_call_id).toBe('c1')
    expect((result[1] as any).tool_call_id).toBe('c2')
  })

  it('still stringifies non-content json tool results unchanged', () => {
    const result = convertToOpenAICompatibleChatMessages([
      {
        role: 'tool',
        content: [
