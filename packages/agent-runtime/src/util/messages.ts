...
export function messagesWithSystem(params: {
  ...
  return [...buildSystemMessagesWithKnowledgeCache(system), ...messages]
}

/**
 * Split a rendered system prompt on the knowledge-files and project-memory
 * sentinels emitted by `templates/strings.ts` and return consecutive
 * `SystemMessage`s — tagged segments get their own cache_control breakpoint
 * via the cache-control pass in `common/src/util/messages.ts`.
 *
 * T0.3: KNOWLEDGE_FILES segment — stable knowledge files cached on breakpoint 2.
 * T0.4: PROJECT_MEMORY segment — project memory cached on breakpoint 3.
 *       The 4th Anthropic cache slot is reserved for tools schema (see
 *       loopAgentSteps in run-agent-step.ts).
 *
 * If all sentinels are absent this returns a single un-tagged system message,
 * preserving the pre-T0.3 behaviour for callers without templates.
 */
export function buildSystemMessagesWithKnowledgeCache(
  system: System,
): SystemMessage[] {
  // `System` here is typed as string. Be defensive in case future callers pass
  // other shapes — fall through to the original single-message behaviour.
  if (typeof system !== 'string') {
    return [systemMessage(system)]
  }

  const kStart = system.indexOf(KNOWLEDGE_FILES_SEGMENT_START)
  const kEnd =
    kStart !== -1
      ? system.indexOf(
          KNOWLEDGE_FILES_SEGMENT_END,
          kStart + KNOWLEDGE_FILES_SEGMENT_START.length,
        )
      : -1

  if (kStart === -1 || kEnd === -1) {
    // No knowledge-files sentinels — try project-memory only
    return splitOnProjectMemory(system)
  }

  const pre = system.slice(0, kStart)
  const knowledge = system.slice(
    kStart + KNOWLEDGE_FILES_SEGMENT_START.length,
    kEnd,
  )
  const post = system.slice(kEnd + KNOWLEDGE_FILES_SEGMENT_END.length)

  const segments: SystemMessage[] = []
  if (pre.trim().length > 0) {
    segments.push(systemMessage(pre))
  }
  ...
  // The post section may contain the project-memory sentinel
  segments.push(...splitOnProjectMemory(post))
...
/**
 * Split a string on the project-memory sentinels (T0.4) and return
 * `SystemMessage`s. The memory segment is tagged PROJECT_MEMORY.
 * If no sentinels are found, returns a single untagged message (or nothing
 * if the text is blank).
 */
function splitOnProjectMemory(text: string): SystemMessage[] {
  if (!text.trim()) return []

  const mStart = text.indexOf(PROJECT_MEMORY_SEGMENT_START)
  const mEnd =
    mStart !== -1
      ? text.indexOf(
          PROJECT_MEMORY_SEGMENT_END,
          mStart + PROJECT_MEMORY_SEGMENT_START.length,
        )
      : -1
  ...
  const pre = text.slice(0, mStart)
  const memory = text.slice(mStart + PROJECT_MEMORY_SEGMENT_START.length, mEnd)
  const post = text.slice(mEnd + PROJECT_MEMORY_SEGMENT_END.length)

  const segments: SystemMessage[] = []
  if (pre.trim()) segments.push(systemMessage(pre))
  if (memory.trim()) {
    segments.push(systemMessage({ content: memory, tags: ['PROJECT_MEMORY'] }))
  }
  if (post.trim()) segments.push(systemMessage(post))

  return segments.length > 0 ? segments : [systemMessage(text)]
}
