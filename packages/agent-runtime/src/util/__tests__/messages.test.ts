...
describe('buildSystemMessagesWithKnowledgeCache (T0.3)', () => {
  it('returns a single system message when sentinels are absent', () => {
    const result = buildSystemMessagesWithKnowledgeCache('Plain system prompt')
    expect(result).toHaveLength(1)
    expect(result[0].role).toBe('system')
    expect(result[0].tags).toBeUndefined()
  })

  it('splits the rendered system into pre / knowledge / post segments', () => {
    const system = [
      'PRE_TEXT',
      `${KNOWLEDGE_FILES_SEGMENT_START}KNOWLEDGE_TEXT${KNOWLEDGE_FILES_SEGMENT_END}`,
      'POST_TEXT',
    ].join('\n')

    const result = buildSystemMessagesWithKnowledgeCache(system)
    expect(result).toHaveLength(3)
    expect(result[0].tags).toBeUndefined()
    expect(result[0].content[0]?.text).toContain('PRE_TEXT')
    expect(result[1].tags).toEqual(['KNOWLEDGE_FILES'])
    expect(result[1].content[0]?.text).toBe('KNOWLEDGE_TEXT')
    expect(result[2].tags).toBeUndefined()
    expect(result[2].content[0]?.text).toContain('POST_TEXT')
  })

  it('drops empty segments around the knowledge block', () => {
    // Sentinels at the very start or end of the rendered system produce empty
    // pre/post segments that should be skipped to avoid emitting empty
    // system messages.
    const system = `${KNOWLEDGE_FILES_SEGMENT_START}KNOWLEDGE_ONLY${KNOWLEDGE_FILES_SEGMENT_END}`
    const result = buildSystemMessagesWithKnowledgeCache(system)
    expect(result).toHaveLength(1)
    expect(result[0].tags).toEqual(['KNOWLEDGE_FILES'])
    expect(result[0].content[0]?.text).toBe('KNOWLEDGE_ONLY')
  })

  it('falls back to a single message when only the start sentinel is present', () => {
    const system = `head ${KNOWLEDGE_FILES_SEGMENT_START} no end here`
    const result = buildSystemMessagesWithKnowledgeCache(system)
    expect(result).toHaveLength(1)
    expect(result[0].tags).toBeUndefined()
    expect(result[0].content[0]?.text).toBe(system)
  })

  it('(T0.4) splits on project memory sentinel alone when no knowledge files', () => {
    const system = [
      'PRE_TEXT',
      `${PROJECT_MEMORY_SEGMENT_START}MEMORY_TEXT${PROJECT_MEMORY_SEGMENT_END}`,
      'POST_TEXT',
    ].join('\n')
    ...
    expect(result[0].tags).toBeUndefined()
    expect(result[0].content[0]?.text).toContain('PRE_TEXT')
    expect(result[1].tags).toEqual(['PROJECT_MEMORY'])
    expect(result[1].content[0]?.text).toBe('MEMORY_TEXT')
    expect(result[2].tags).toBeUndefined()
    expect(result[2].content[0]?.text).toContain('POST_TEXT')
  })

  it('(T0.4) splits on both knowledge files and project memory sentinels', () => {
    const system = [
      'PRE',
    ...
    const result = buildSystemMessagesWithKnowledgeCache(system)
    // Expect: pre, knowledge (tagged), mid+memory_pre (plain), memory (tagged), post (plain)
    const taggedSegments = result.filter((s) => s.tags && s.tags.length > 0)
    expect(taggedSegments).toHaveLength(2)
    const knowledgeSeg = result.find((s) => s.tags?.includes('KNOWLEDGE_FILES'))
    const memorySeg = result.find((s) => s.tags?.includes('PROJECT_MEMORY'))
    expect(knowledgeSeg).toBeDefined()
    expect(knowledgeSeg!.content[0]?.text).toBe('KNOWLEDGE')
    expect(memorySeg).toBeDefined()
    expect(memorySeg!.content[0]?.text).toBe('MEMORY')
  })

  it('(T0.4) drops empty project memory segment', () => {
    const system = [
      'PRE',
      `${KNOWLEDGE_FILES_SEGMENT_START}KNOWLEDGE${KNOWLEDGE_FILES_SEGMENT_END}`,
    ...
    const result = buildSystemMessagesWithKnowledgeCache(system)
    const memorySeg = result.find((s) => s.tags?.includes('PROJECT_MEMORY'))
    expect(memorySeg).toBeUndefined()
  })
})
