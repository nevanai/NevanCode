...
describe('getAgentPrompt', () => {
  ...
  describe('PROJECT_MEMORY placeholder', () => {
    test('renders memory contents wrapped in a .nevan/memory.md code fence', async () => {
      const memoryBody =
        '# Project Memory\n\n## Facts\n- runs on port 5001 (frontend) and 8080 (backend)'
      const fileContext: ProjectFileContext = {
        ...createMockFileContext(),
        projectMemory: memoryBody,
      }

      const template = createMockAgentTemplate({
        id: 'main',
        systemPrompt: 'preamble\n{CODEBUFF_PROJECT_MEMORY}\npostamble',
      })
      ...
      const result = await getAgentPrompt({
        ...
        promptType: { type: 'systemPrompt' },
        fileContext,
        agentState: createMockAgentState('main'),
        agentTemplates: { main: template },
        additionalToolDefinitions: async () => ({}),
        logger: createMockLogger(),
        apiKey: TEST_AGENT_RUNTIME_IMPL.apiKey,
        databaseAgentCache: TEST_AGENT_RUNTIME_IMPL.databaseAgentCache,
      ...
      expect(result).toBeDefined()
      expect(result).toContain('```.nevan/memory.md')
      expect(result).toContain('runs on port 5001 (frontend) and 8080 (backend)')
      expect(result).not.toContain('{CODEBUFF_PROJECT_MEMORY}')
    })

    test('renders empty string when no memory is present', async () => {
      const template = createMockAgentTemplate({
        id: 'main',
        systemPrompt: 'start[{CODEBUFF_PROJECT_MEMORY}]end',
      })
      ...
      expect(result).not.toContain('```.nevan/memory.md')
    })

    test('renders empty string when memory is whitespace-only', async () => {
      const fileContext: ProjectFileContext = {
        ...createMockFileContext(),
        projectMemory: '   \n\t  \n',
      }
