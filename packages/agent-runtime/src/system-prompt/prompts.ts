...
**لا تقرأ الملفات يدويًا أبدًا.** بدلاً من ذلك، استخدم ملفات المعرفة هذه كجزء من الفهرسة الجاهزة للمشروع لفهم قاعدة الكود بذكاء.
...
// T0.3 sentinels: mark the knowledge-files segment inside a rendered system
// prompt. The string template in `templates/strings.ts` wraps the rendered
// KNOWLEDGE_FILES_CONTENTS with these tokens; `run-agent-step.ts` then splits
// the rendered system on them so the knowledge files become a separate
// `SystemMessage` with its own cache_control breakpoint. Strings are
// distinctive enough that they will not collide with user-authored knowledge
// content; if a future knowledge file genuinely contained these tokens, the
// split would still be safe (it would just produce a slightly larger
// knowledge segment).
export const KNOWLEDGE_FILES_SEGMENT_START =
  '<<<NEVAN_KNOWLEDGE_FILES_SEGMENT_START>>>'
export const KNOWLEDGE_FILES_SEGMENT_END =
  '<<<NEVAN_KNOWLEDGE_FILES_SEGMENT_END>>>'

// T0.4 sentinels: mark the project-memory segment (`.nevan/memory.md`) so it
// gets its own cache_control breakpoint, separate from the knowledge files and
// the volatile tail (system info, git changes). Project memory is stable for
// the duration of a session and worth caching on its own breakpoint.
export const PROJECT_MEMORY_SEGMENT_START =
  '<<<NEVAN_PROJECT_MEMORY_SEGMENT_START>>>'
export const PROJECT_MEMORY_SEGMENT_END =
  '<<<NEVAN_PROJECT_MEMORY_SEGMENT_END>>>'
...
As Nevan Code, you have access to all the files in the project.
...
لا تقرأ المشروع يدويًا بنفسك. حلل الفهرسة الجاهزة للمشروع (خاصة قسم \`<relevant_files>\` إذا كان متاحًا) لفهمه بذكاء.
${
  mode === 'agent'
...
IMPORTANT: Your training data has a knowledge cutoff. The current year is ${currentYear}. Always use web_search (via the researcher-web agent) to verify current library versions, recent framework changes, new APIs, or any information that could have changed after your training cutoff. Do NOT assume the latest version of any library or technology from your training data is still current.

/Smartsearch: ALWAYS include the current year (${currentYear}) in web search queries about libraries, frameworks, or any technology. This ensures search engines return current results and the /Smartsearch protocol filters out stale pre-2024 information. Never search for version numbers without a year qualifier. Example: search "React latest stable version ${currentYear}" instead of "React version".
