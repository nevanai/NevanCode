...
// Long default so the web-search backend never aborts a multi-hour run.
// Override with LINKUP_TIMEOUT_MS or disable via CODEBUFF_DISABLE_TIMEOUTS=1.
const FETCH_TIMEOUT_MS = (() => {
  const envValue =
    typeof process !== 'undefined'
      ? process.env?.LINKUP_TIMEOUT_MS
      : undefined
