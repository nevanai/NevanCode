...
function ensureJsonSchemaCompatible(schema: z.ZodType): z.ZodType {
  try {
    ...
    // Falling back to an empty schema means the model sees a tool with no
    // parameters and can't call it correctly. This is silent by default, which
    // previously hid a Zod-mangling bug for a long time — so make it loud.
    // eslint-disable-next-line no-console
    console.warn(
      `[tools] Failed to convert a tool schema to JSON Schema; the model will see NO parameters for it. ${
...
function toJsonSchemaSafe(schema: z.ZodType): Record<string, unknown> {
  try {
    ...
    // Same trap as ensureJsonSchemaCompatible: an empty fallback hides the tool's
    // real parameters from the model. Surface it instead of failing silently.
    // eslint-disable-next-line no-console
    console.warn(
      `[tools] Failed to render a tool's params as JSON Schema; its parameters will be omitted from the prompt. ${
        error instanceof Error ? error.message : String(error)
      }`,
    )
...
You (Nevan Code) have access to the following tools. Call them when needed.
      ...
      example_array: [
        {
          ...
          new_content_with_newlines:
            "// some context\nconsole.log('Hello from Nevan Code!');\n",
        },
      ],
    },
    false,
...
export async function getToolSet(params: {
  ...
  for (const [toolName, toolDefinition] of Object.entries(toolDefinitions)) {
    // IMPORTANT: do NOT cloneDeep the definition. For MCP tools `inputSchema` is
    // a Zod (v4) schema, and lodash cloneDeep mangles its internal `_zod.def`,
    // which makes z.toJSONSchema throw. ensureJsonSchemaCompatible then silently
    // falls back to an empty `z.object({})`, so the model receives a tool with
    // `properties: {}` — it can't tell what arguments to pass, calls the tool
    // with empty input, gets a validation error, and retries for minutes.
    // Spreading into a new object below already avoids mutating the original.
