...
export const handleWebSearch = (async (params: {
  ...
  try {
    ...
    // /Smartsearch protocol: log every search to a structured JSONL file.
    // Best-effort only — file I/O failures must not affect the tool response.
    try {
      const now = new Date()
      const dateStr = now.toISOString().split('T')[0]
      const logDir = join(process.cwd(), '.nevan', 'search-log')
      if (!existsSync(logDir)) {
        mkdirSync(logDir, { recursive: true })
      }
      const logPath = join(logDir, `search-${dateStr}.jsonl`)
