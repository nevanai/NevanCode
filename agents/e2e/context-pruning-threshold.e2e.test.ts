...
describe('Context Pruning Threshold E2E', () => {
  it(
    ...
    async () => {
      ...
      // With maxContextLength=100k the 80% compaction threshold is 80k, so ~30k
      // of content (plus overhead) stays well below it and must not prune.
  ...
  it(
    ...
    async () => {
      ...
      // With maxContextLength=50k the 80% compaction threshold is 40k, so ~80k
      // of content comfortably exceeds it and must prune.
  ...
  it(
    ...
    async () => {
      ...
      // Auto-compaction triggers at 80% of the window (see context-pruner's
      // COMPACTION_THRESHOLD_RATIO), so the boundary we compare against is the
      // effective threshold, not the raw limit.
      const COMPACTION_THRESHOLD_RATIO = 0.8
      const EFFECTIVE_THRESHOLD = MAX_CONTEXT_LENGTH * COMPACTION_THRESHOLD_RATIO
      ...
      // =========================================================================
      // DIAGNOSIS: Compare true tokens vs the 80% compaction threshold
      // =========================================================================
      if (trueTokenCount < EFFECTIVE_THRESHOLD && pruningResult.wasPruned) {
        console.error(
          `  ❌ BUG DETECTED: True tokens (${trueTokenCount}) < threshold (${EFFECTIVE_THRESHOLD}), ` +
            `but pruning was triggered! The token counting API is over-reporting.`,
        )
      } else if (
        trueTokenCount < EFFECTIVE_THRESHOLD &&
        !pruningResult.wasPruned
      ) {
        console.log(
          `  ✅ No bug: True tokens (${trueTokenCount}) < threshold (${EFFECTIVE_THRESHOLD}), ` +
            `no pruning occurred.`,
        )
      } else {
        console.log(
          `  ⚠️ Content over threshold: True tokens (${trueTokenCount}) >= threshold (${EFFECTIVE_THRESHOLD}). ` +
            `Pruning is expected.`,
        )
      }
      ...
      // CRITICAL: If true tokens are under the 80% threshold (80k), no pruning
      // should have occurred. If true tokens >= the threshold, pruning is
      // expected and we skip this assertion.
      if (trueTokenCount < EFFECTIVE_THRESHOLD) {
        expect(pruningResult.wasPruned).toBe(false)
      } else {
        console.log(
          `  [accuracy] Content over threshold: true tokens (${trueTokenCount}) >= threshold (${EFFECTIVE_THRESHOLD}). Pruning is expected.`,
        )
      }
    },
    { timeout: 300_000 },
