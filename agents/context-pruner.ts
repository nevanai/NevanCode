...
const definition: AgentDefinition = {
  ...
  handleSteps: function* ({ agentState, params }) {
    ...
    /**
     * Fraction of the context window at which we compact AUTOMATICALLY.
     *
     * Compaction kicks in once usage crosses this fraction of maxContextLength
     * (default 80%), instead of waiting for the window to fill completely. This
     * keeps the live model context comfortably below the hard limit so a long
     * session never races the provider's real token count into a "prompt is too
     * long" error, and the user-visible context bar recovers well before 100%.
     * Overridable per-spawn via params.compactionThresholdRatio.
     */
    const COMPACTION_THRESHOLD_RATIO: number =
      typeof params?.compactionThresholdRatio === 'number' &&
      params.compactionThresholdRatio > 0 &&
      params.compactionThresholdRatio <= 1
        ? params.compactionThresholdRatio
        : 0.8
    ...
    /**
     * Sliding window over screenshots so a session can run forever without the
     * stored history growing unbounded.
     *
     * Tool-result images (computer-use / browser screenshots) are huge — a
     * single resized screenshot is ~0.5MB of base64. Left untouched, every
     * screenshot stays in messageHistory: tokens, the per-turn wire payload, AND
     * the on-disk `saveChatState` snapshot all grow without limit. The model
     * only needs the CURRENT screen plus its own (text) reasoning about earlier
     * ones, so we keep the last `keepLastImages` screenshot images intact and
     * replace the base64 of older ones with a short text placeholder, written
     * back into the stored history.
     *
     * This runs every step (not only at the prune threshold) so base64 never
     * accumulates between prunes. It mutates only the conversation tail's image
     * payloads — the cached system/tools/knowledge prefix is untouched.
     *
     * Returns a NEW array; input messages are not mutated.
     */
    function applyScreenshotWindow(
      msgs: Message[],
      keepLastImages: number,
    ): Message[] {
      // Identify image-bearing content parts inside tool/user messages, newest
      // first, and mark every part beyond the keep budget for stripping.
      const isImagePart = (part: any): boolean =>
        part &&
        typeof part === 'object' &&
        (part.type === 'media' || part.type === 'image') &&
        typeof (part.data ?? part.image) === 'string' &&
        (part.data ?? part.image).length > 0

      // Collect (messageIndex, partIndex) of every image part in order.
      const imageLocations: Array<{ m: number; p: number }> = []
      for (let m = 0; m < msgs.length; m++) {
        const content = msgs[m].content
        if (!Array.isArray(content)) continue
        for (let p = 0; p < content.length; p++) {
          if (isImagePart(content[p])) {
            imageLocations.push({ m, p })
          }
        }
      }

      // Nothing to do if we're within budget.
      if (imageLocations.length <= keepLastImages) {
        return msgs
      }

      // Keep the last `keepLastImages`; strip the rest.
      const stripCount = imageLocations.length - keepLastImages
      const toStrip = new Set<string>()
      for (let i = 0; i < stripCount; i++) {
        toStrip.add(`${imageLocations[i].m}:${imageLocations[i].p}`)
      }

      const PLACEHOLDER = '[older screenshot removed to save context]'

      let stripped = 0
      const result = msgs.map((message, m) => {
        const content = message.content
        if (!Array.isArray(content)) return message
        let changed = false
        const newContent = content.map((part: any, p: number) => {
          if (!toStrip.has(`${m}:${p}`)) return part
          changed = true
          stripped++
          // Replace the image with a compact placeholder whose SHAPE stays valid
          // for the parent message's role. Tool-result content parts may only be
          // `json` or `media` (see convertToolResultMessage); user/assistant
          // content parts use `text`. Emitting the wrong shape throws downstream
          // ("Invalid tool output type: text"), so branch on role.
          if (message.role === 'tool') {
            return { type: 'json' as const, value: PLACEHOLDER }
          }
    ...
    // How many of the most-recent screenshots keep their full image. The model
    // needs the current screen; older screens are preserved as text reasoning.
    const keepLastImages: number = params?.keepLastImages ?? 3
    ...
    // Bound screenshot base64 in the STORED history every step (independent of
    // the token/cache prune below). Without this, a long computer-use session
    // accumulates ~0.5MB/screenshot in tokens, wire bytes, and the saveChatState
    // snapshot — eventually "prompt is too long". Keeping only the last N images
    // makes the session runnable indefinitely.
    currentMessages = applyScreenshotWindow(currentMessages, keepLastImages)
    ...
    // Compact once usage crosses COMPACTION_THRESHOLD_RATIO of the window
    // (default 80%), not only when it is completely full. Summarizing earlier
    // keeps the real model context bounded so the window never overflows.
    const compactionThreshold = Math.floor(
      maxContextLength * COMPACTION_THRESHOLD_RATIO,
    )

    // Check if we need to prune at all:
    // - Prune when context crosses the compaction threshold (80% of max), OR
