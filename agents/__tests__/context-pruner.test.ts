...
describe('context-pruner handleSteps', () => {
  ...
  test('does nothing when context is under the compaction threshold', () => {
    const messages = [
      createMessage('user', 'Hello'),
      createMessage('assistant', 'Hi there!'),
    ]

    // Context under the 80% compaction threshold (160k of 200k) - no pruning
    const results = runHandleSteps(messages, 150000, 200000)
...
describe('context-pruner threshold behavior', () => {
  ...
  test('does not prune just below the 80% compaction threshold', () => {
    const messages = [
      createMessage('user', 'Hello'),
      createMessage('assistant', 'Hi'),
    ]

    // 80% of 200000 = 160000. With the 1000 fudge factor the boundary for NOT
    // pruning is contextTokenCount + 1000 <= 160000, i.e. contextTokenCount <=
    // 159000. Right at 159000 we must still preserve the conversation.
    const results = runHandleSteps(messages, 159000, 200000)
  ...
  test('prunes once the 80% compaction threshold is crossed', () => {
    const messages = [
      createMessage('user', 'Hello'),
      createMessage('assistant', 'Hi'),
    ]

    // At 160000 (exactly 80% of 200000) the fudge factor pushes us over the
    // threshold (160000 + 1000 > 160000), so auto-compaction must trigger well
    // before the window is full.
    const results = runHandleSteps(messages, 160000, 200000)
  ...
  test('honors a custom compactionThresholdRatio', () => {
    const messages = [
      createMessage('user', 'Hello'),
      createMessage('assistant', 'Hi'),
    ]

    // With a 0.5 ratio the threshold becomes 100000. 120000 is under the full
    // 200000 window but over the custom threshold, so it must compact.
    const results = runHandleSteps(messages, 120000, 200000, {
...
describe('context-pruner screenshot window (unbounded sessions)', () => {
  // A screenshot tool-result as computer-use / browser MCP produces it.
  const screenshotMsg = (id: string, b64: string): ToolMessage => ({
    role: 'tool',
    toolCallId: id,
    toolName: 'computer_take_screenshot',
    content: [
      { type: 'json', value: 'Screenshot captured.' },
      { type: 'media', data: b64, mediaType: 'image/jpeg' } as any,
    ],
  })

  const runPruner = (messages: Message[], params: Record<string, unknown>) => {
    const agentState = createMockAgentState(
      messages,
      // Keep contextTokenCount LOW so the fast (non-summarizing) path runs —
      // this proves the screenshot window applies on every step, not only at
      // the prune threshold.
      10_000,
    )
    const generator = contextPruner.handleSteps!({
    ...
    while (!r.done) {
      if (typeof r.value === 'object') results.push(r.value)
      r = generator.next()
    }
    return results
  }

  const countImages = (messages: Message[]): number => {
    let count = 0
  ...
  const buildHistory = (k: number, b64: string): Message[] => {
    const history: Message[] = []
    for (let i = 0; i < k; i++) {
      history.push(createToolCallMessage('c' + i, 'computer_take_screenshot', {}))
      history.push(screenshotMsg('c' + i, b64))
    }
    return history
  }

  test('keeps only the last N screenshots, even on the fast path', () => {
    const b64 = 'Z'.repeat(20000)
    const results = runPruner(buildHistory(10, b64), { keepLastImages: 3 })
    expect(results).toHaveLength(1)
    expect(results[0].toolName).toBe('set_messages')
    const out: Message[] = results[0].input.messages
    expect(countImages(out)).toBe(3)
  })

  test('preserves the current screen (most recent screenshot stays a full image)', () => {
    const b64 = 'Z'.repeat(20000)
    const history = buildHistory(8, b64)
    const results = runPruner(history, { keepLastImages: 2 })
    const out: Message[] = results[0].input.messages
    // The last tool message must still carry a media part.
    const lastToolWithMedia = [...out]
      .reverse()
      .find(
        (m) =>
          m.role === 'tool' &&
          Array.isArray(m.content) &&
          (m.content as any[]).some((p) => p.type === 'media'),
      )
    expect(lastToolWithMedia).toBeDefined()
  })

  test('replaces older screenshots with a text placeholder (no base64 retained)', () => {
    const bigB64 = 'A'.repeat(50000)
    const results = runPruner(buildHistory(5, bigB64), { keepLastImages: 1 })
    const out: Message[] = results[0].input.messages
    const serialized = JSON.stringify(out)
    // Only 1 copy of the base64 may remain (the current screen).
    const occurrences = serialized.split(bigB64).length - 1
    expect(occurrences).toBe(1)
    expect(serialized).toContain('older screenshot removed')
  })

  test('bounds stored history regardless of screenshot count (invariant at K=200)', () => {
    const b64 = 'Z'.repeat(20000)
    const results = runPruner(buildHistory(200, b64), { keepLastImages: 3 })
    const out: Message[] = results[0].input.messages
    expect(countImages(out)).toBe(3)
  })

  test('defaults to keeping 3 screenshots when keepLastImages is not set', () => {
    const b64 = 'Z'.repeat(20000)
    const results = runPruner(buildHistory(10, b64), {})
    const out: Message[] = results[0].input.messages
    expect(countImages(out)).toBe(3)
  })

  test('leaves history untouched when under the image budget', () => {
    const b64 = 'Z'.repeat(20000)
    const history = buildHistory(2, b64)
    const results = runPruner(history, { keepLastImages: 3 })
    const out: Message[] = results[0].input.messages
