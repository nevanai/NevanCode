  ...
  try {
    const res = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        // Structured system block with cache_control: the prompt is ~80 lines
        // of stable rules and repeats every sweep, so caching it is a 5–10×
        // cost win on this code path. (Claude 4.x: prompt caching is GA, no
        // beta header needed.)
        system: [
          {
            type: 'text',
