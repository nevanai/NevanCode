...
describe('formatToolResultBlocks', () => {
  ...
  it('returns a string for text-only tool results', () => {
    const content = [{ type: 'json', value: 'file written' }]
    expect(formatToolResultBlocks(content)).toBe('"file written"')
  })

  it('emits an image block (not base64 text) for screenshot media results', () => {
    const content = [
      { type: 'json', value: 'Screenshot captured.' },
      { type: 'media', data: b64, mediaType: 'image/jpeg' },
    ]
    const result = formatToolResultBlocks(content)
  ...
  it('does not place raw base64 in any text block', () => {
    const bigB64 = 'A'.repeat(50000)
    const content = [{ type: 'media', data: bigB64, mediaType: 'image/png' }]
    const result = formatToolResultBlocks(content) as any[]
    const textBlocks = result.filter((b) => b.type === 'text')
    for (const tb of textBlocks) {
      expect(tb.text).not.toContain(bigB64)
    }
    // base64 lives only inside the image source
    const imageBlock = result.find((b) => b.type === 'image')
    expect(imageBlock.source.data).toBe(bigB64)
  })

  it('also handles the `image` part shape (not just `media`)', () => {
    const content = [{ type: 'image', image: b64, mediaType: 'image/png' }]
    const result = formatToolResultBlocks(content) as any[]
