...
export function convertToAnthropicMessages(
  ...
  for (const message of messages) {
    ...
    // Anthropic's tool_result accepts an array of text + image blocks, so image
    // results (computer-use / browser screenshots) are counted as images
    // (~1600 tokens) rather than as their raw base64 (~150K tokens of text).
    // This must mirror the send path (the OpenRouter fork relocates images to a
    // following user message) so token-count and the real request agree —
    // otherwise the context-pruner over- or under-fires.
    if (message.role === 'tool') {
      result.push({
        role: 'user',
        content: [
          {
            ...
            content: formatToolResultBlocks(message.content),
          },
        ],
      })
      continue
...
/**
 * Builds Anthropic tool_result content. Returns a plain string when there are
 * no images (cheaper, and matches the common case), or an array of text + image
 * blocks when the tool returned media — so screenshots are counted as images,
 * not as ~150K tokens of base64 text. The base64 lives under `data` (media) or
 * `image` (image) on each content part.
 */
export function formatToolResultBlocks(content: any): string | any[] {
  if (typeof content === 'string') {
    return content
  }
  if (!Array.isArray(content)) {
  ...
  if (!hasImage) {
    return formatToolContent(content)
  }
