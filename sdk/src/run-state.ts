...
import { readProjectMemory } from '@codebuff/common/util/project-memory'
...
export async function initialSessionState(
  ...
  // Load persistent project memory (.nevan/memory.md). Missing file is fine.
  const projectMemory = cwd
    ? await readProjectMemory({ projectRoot: cwd, fs, logger })
    : undefined
