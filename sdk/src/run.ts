...
import { getChatGptOAuthCredentials, isChatGptOAuthValid, getUserCredentials, isAnyByokProviderLinked, getAnthropicOAuthCredentials, getGeminiOAuthCredentials } from './credentials'
...
async function runOnce({
  ...
  // When ChatGPT OAuth, a Claude account, or a BYOK provider is active, skip
  // the Codebuff backend user lookup — requests route directly to the provider
  // and don't need a Codebuff user ID. (Without this, a Claude-connected user
  // with no Codebuff account fails the whole run on a backend "Network request
  // failed" before routing ever reaches the native Anthropic API.)
  let userId: string
  if (
    (getChatGptOAuthCredentials() && isChatGptOAuthValid()) ||
    isAnyByokProviderLinked() ||
    getAnthropicOAuthCredentials() ||
    getGeminiOAuthCredentials()
  ) {
    const storedUser = getUserCredentials()
    userId =
      storedUser?.id ??
      (isAnyByokProviderLinked()
        ? 'byok-user'
        : getAnthropicOAuthCredentials()
          ? 'anthropic-oauth-user'
          : getGeminiOAuthCredentials()
            ? 'gemini-oauth-user'
            : 'chatgpt-oauth-user')
  } else {
