...
describe('StatusIndicator state logic', () => {
  describe('getStatusIndicatorState', () => {
    ...
    test('does not surface retrying when auth is retrying — folds into underlying stream status', () => {
      const state = getStatusIndicatorState({
        ...baseArgs,
        isConnected: true,
        authStatus: 'retrying',
    ...
    test('does not surface retrying when message send is retrying — folds into underlying stream status', () => {
      const state = getStatusIndicatorState({
        ...baseArgs,
        isRetrying: true,
    ...
    test('does not surface connecting when not connected — folds into underlying stream status', () => {
      const state = getStatusIndicatorState({
        ...baseArgs,
        isConnected: false,
        streamStatus: 'streaming',
    ...
    test('does not surface connecting when auth service is unreachable — folds into underlying stream status', () => {
      const state = getStatusIndicatorState({
        ...baseArgs,
        isConnected: true,
    ...
    test('does not surface connecting when both WebSocket and auth service are unreachable', () => {
      const state = getStatusIndicatorState({
        ...baseArgs,
        isConnected: false,
        authStatus: 'unreachable',
    ...
    describe('state priority order', () => {
      ...
      test('clipboard still wins over disconnection (which itself no longer surfaces)', () => {
        const state = getStatusIndicatorState({
          ...baseArgs,
          statusMessage: 'Test',
          isConnected: false,
      ...
      test('retrying auth no longer overrides waiting', () => {
        const state = getStatusIndicatorState({
          ...baseArgs,
          isConnected: true,
          authStatus: 'retrying',
      ...
      test('disconnection no longer overrides waiting', () => {
        const state = getStatusIndicatorState({
          ...baseArgs,
          isConnected: false,
          streamStatus: 'waiting',
      ...
      test('auth unreachable no longer overrides waiting', () => {
        const state = getStatusIndicatorState({
          ...baseArgs,
          isConnected: true,
          authStatus: 'unreachable',
