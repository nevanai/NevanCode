...
import { getSelectedOpenAiRuntimeStatus } from '../utils/openai-runtime'
...
export const UsageBanner = ({ showTime }: { showTime: number }) => {
  ...
  const openAiRuntimeStatus = getSelectedOpenAiRuntimeStatus()
  const selectedOpenAiModel = openAiRuntimeStatus.selectedModel

  // Check if ChatGPT OAuth is connected
  const isChatGptConnected =
    CHATGPT_OAUTH_ENABLED && openAiRuntimeStatus.connection.connected
  ...
  return (
    ...
    >
      <box style={{ flexDirection: 'column', gap: 0 }}>
        ...
        {(selectedOpenAiModel || isChatGptConnected) && (
          <box style={{ flexDirection: 'column', marginTop: 1 }}>
            <text style={{ fg: theme.muted }}>OpenAI routing</text>
            {selectedOpenAiModel ? (
              <>
                <text style={{ fg: theme.muted }}>
                  Selected model: {selectedOpenAiModel.label}
                </text>
                <text style={{ fg: theme.muted }}>
                  {openAiRuntimeStatus.connection.state === 'connected'
                    ? 'ChatGPT connected. New messages use this model.'
                    : openAiRuntimeStatus.connection.state === 'expired'
                      ? 'ChatGPT connection expired. Reconnect via /connect.'
                      : 'Connect via /connect to use this model.'}
                </text>
              </>
            ) : (
              <text style={{ fg: theme.muted }}>
                ChatGPT connected for supported OpenAI models
              </text>
            )}
          </box>
        )}
