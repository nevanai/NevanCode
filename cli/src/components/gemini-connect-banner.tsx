...
import {
  type GeminiOAuthConnectionState,
  connectGeminiOAuth,
...
export const GeminiConnectBanner = () => {
  const theme = useTheme()
  const setInputMode = useChatStore((state) => state.setInputMode)
  const [flowState, setFlowState] = useState<FlowState>('checking')
  const [connectionState, setConnectionState] =
    useState<GeminiOAuthConnectionState>('not-connected')
  const [error, setError] = useState<string | null>(null)
  const [authUrl, setAuthUrl] = useState<string | null>(null)
  const [hovered, setHovered] = useState(false)
  const [isCloseHovered, setIsCloseHovered] = useState(false)

  const beginConnection = (state: GeminiOAuthConnectionState) => {
    setConnectionState(state === 'connected' ? 'not-connected' : state)
    setFlowState('waiting-for-code')
    setError(null)
    const result = connectGeminiOAuth()
    setAuthUrl(result.authUrl)
  }

  useEffect(() => {
    const status = getGeminiOAuthStatus()
  ...
  const handleConnect = () => {
    beginConnection(getGeminiOAuthStatus().state)
  }
    ...
    return (
      ...
      >
        <box style={{ flexDirection: 'column', flexShrink: 1 }}>
          <text style={{ fg: theme.foreground }}>✓ Gemini connected</text>
          <text style={{ fg: theme.muted }}>
            Your Google account now answers every request.
          </text>
        </box>
    ...
    return (
      ...
      >
        <box style={{ flexDirection: 'column', flexShrink: 1 }}>
          ...
          <text style={{ fg: theme.muted }}>
            Reconnect to use your Google account.
          </text>
        </box>
        <box style={{ flexDirection: 'row', gap: 1, alignItems: 'center' }}>
          <Button
  ...
  if (flowState === 'waiting-for-code') {
    ...
    return (
      <box style={{ ...panelStyle, flexDirection: 'column' }}>
        ...
        <text style={{ fg: theme.muted }}>
          1. Authorize in your browser (Google account).
        </text>
        <text style={{ fg: theme.muted }}>
    ...
    return (
      ...
      >
        <box style={{ flexDirection: 'column', gap: 1 }}>
          <text style={{ fg: theme.muted }}>
            Connect your Google account to run every model on your Gemini
            subscription.
          </text>
          <Button
            style={actionButtonStyle}
          ...
          >
            <text wrapMode="none">
              <span fg={theme.link}>Connect to Gemini</span>
            </text>
          </Button>
        </box>
        {closeButton}
...
export async function handleGeminiAuthCode(code: string): Promise<{
  success: boolean
  message: string
}> {
  try {
    await exchangeGeminiCodeForTokens(code)
    return {
      success: true,
      message:
        'Successfully connected your Google account! NevanCode will route every model through your Gemini account.',
    }
  } catch (err) {
    return {
      success: false,
      message:
        err instanceof Error
          ? err.message
          : 'Failed to exchange Gemini authorization code',
    }
  }
}
