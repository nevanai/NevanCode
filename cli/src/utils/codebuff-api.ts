...
export function createCodebuffApiClient(
  ...
  // Long default so a slow backend never kills sessions that run for hours.
  // Override per-request via RequestOptions.timeoutMs, per-client via
  // CodebuffApiClientConfig.defaultTimeoutMs, or globally via env vars
  // CODEBUFF_API_TIMEOUT_MS / CODEBUFF_DISABLE_TIMEOUTS=1.
  const envTimeout =
    typeof process !== 'undefined'
      ? Number(process.env?.CODEBUFF_API_TIMEOUT_MS)
      : NaN
  ...
  const {
    ...
    authToken,
    fetch: fetchFn = fetch,
    defaultTimeoutMs = envTimeoutValid ? envTimeout : 60 * 60 * 1000,
  ...
  async function request<T>(
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      // Create abort controller for timeout (skipped when timeouts are disabled
      // — used for sessions that must run for many hours/days uninterrupted).
      const controller = new AbortController()
      const timeoutId: NodeJS.Timeout | undefined = timeoutsDisabled
        ? undefined
