...
function callbackPageHtml(success: boolean, errorMessage?: string): string {
  const title = success
    ? 'Connected — NevanCode'
    : 'Connection Failed — NevanCode'
  const heading = success ? '✓ Connected to ChatGPT' : 'Connection Failed'
  const body = success
    ? 'You can close this tab and return to NevanCode.'
    : `${escapeHtml(errorMessage ?? 'Unknown error')}. Return to NevanCode and try /connect:chatgpt again.`
  return `<!DOCTYPE html>
...
<h1 style="color:#ffffff;margin-bottom:0.5rem">${heading}</h1>
<p style="color:#bfbfbf">${body}</p>
</div></body></html>`
}
...
export async function exchangeChatGptCodeForTokens(
  ...
  // Connecting ChatGPT/Codex makes it the sole active OAuth provider — clear the
  // others (both are checked before ChatGPT in routing and /model).
  clearGeminiOAuthCredentials()
  clearAnthropicOAuthCredentials()
  resetChatGptOAuthRateLimit()
  pendingCodeVerifier = null
...
export type ChatGptOAuthConnectionState =
  | 'not-connected'
  | 'expired'
  | 'connected'
...
export function getChatGptOAuthStatus(): {
  connected: boolean
  state: ChatGptOAuthConnectionState
  expiresAt?: number
  connectedAt?: number
} {
