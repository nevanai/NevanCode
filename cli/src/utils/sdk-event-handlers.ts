...
export const createEventHandler =
  (state: EventHandlerState) => (event: SDKEvent) => {
    return match(event)
      ...
      .with({ type: 'context_usage' }, (e) => {
        // Live real model context for the bar — updates mid-run so the user
        // sees the actual token count rise and drop on auto-compaction.
        useChatStore.getState().setLiveContextUsage({
          tokenCount: e.contextTokenCount,
          maxTokens: e.maxContextTokens,
        })
      })
      .otherwise(() => undefined)
  }
