...
describe('markdown renderer', () => {
  ...
  test('renders inline code with palette colors', () => {
    ...
    expect(inlineCode.props.fg).toBe('#ffffff')
    // codeMonochrome palette omits the background swatch
    expect(inlineCode.props.bg).toBeUndefined()
  ...
  test('renders headings with color and bold attribute', () => {
    ...
    expect(heading.props.fg).toBe('#ffffff')
  ...
  test('renders lists with bullet markers', () => {
    ...
    bulletSpans.forEach((span) => expect(span.props.fg).toBe('#bfbfbf'))
...
\`\`\``
  ...
  test('renders inline code followed by quotes correctly', () => {
    const markdown = 'Use `nevancode "fix bug"` to fix bugs.'
    const output = renderMarkdown(markdown)
    const nodes = flattenNodes(output)
    ...
    expect(inlineContent).toContain('nevancode "fix bug"')

    expect(nodes[2]).toBe(' to fix bugs.')

    // Verify quotes are inside the inline code, not concatenated after
    expect(inlineContent).toMatch(/nevancode\s+"fix bug"/)
  })
