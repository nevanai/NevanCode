...
describe('sdk-event-handlers', () => {
  ...
  test('context_usage event updates live context usage in the store', () => {
    const { ctx } = createTestContext()
    const handleEvent = createEventHandler(ctx)

    useChatStore.getState().setLiveContextUsage(null)
    handleEvent({
      type: 'context_usage',
      contextTokenCount: 123_456,
      maxContextTokens: 200_000,
      agentId: 'main-agent',
    } as any)

    expect(useChatStore.getState().liveContextUsage).toEqual({
      tokenCount: 123_456,
      maxTokens: 200_000,
    })

    // Cleanup so the global store doesn't leak into other tests.
    useChatStore.getState().setLiveContextUsage(null)
  })
})
