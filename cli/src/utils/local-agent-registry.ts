...
// ============================================================================
// Built-in MCP server — auto-registered, no configuration required
//
// Publicly registered as `nevancode-mcp` — this is the name agents and the UI
// see (and what tool calls are namespaced under). The underlying implementation
// is still the Hermes MCP package in packages/codebuff-hermes-mcp; only the
// registered name is branded.
// ============================================================================

const BUILTIN_MCP_SERVER_NAME = 'nevancode-mcp'

/**
 * Finds the hermes-mcp server entry point by checking known locations
 * relative to the CLI source file and the current working directory.
 * Returns the absolute path to index.ts, or null if not found.
 */
function findHermesMcpServerPath(): string | null {
  const candidates = [
    // Running from monorepo root (bun start-cli)
    path.join(process.cwd(), 'packages/codebuff-hermes-mcp/src/index.ts'),
    // Running from cli/ directory
    path.join(process.cwd(), '../packages/codebuff-hermes-mcp/src/index.ts'),
    // Resolved from this source file location (cli/src/utils/ → monorepo root)
    path.join(import.meta.dir, '../../../packages/codebuff-hermes-mcp/src/index.ts'),
  ]
...
export async function initializeAgentRegistry(): Promise<void> {
  ...
  // Auto-register the built-in MCP server under its branded name.
  // This runs unconditionally — no mcp.json or manual configuration required.
  // The server is started on-demand the first time one of its tools is called.
  if (!mcpServersCache[BUILTIN_MCP_SERVER_NAME]) {
    const hermesMcpPath = findHermesMcpServerPath()
    if (hermesMcpPath) {
      mcpServersCache[BUILTIN_MCP_SERVER_NAME] = {
        type: 'stdio',
        command: 'bun',
        args: [hermesMcpPath],
        env: {},
      ...
      logger.debug(
        { path: hermesMcpPath, registeredAs: BUILTIN_MCP_SERVER_NAME },
        '[agents] Auto-registered built-in MCP server',
      )
    }
  }
}
