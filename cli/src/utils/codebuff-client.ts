...
export function getToolDisplayInfo(toolName: string): {
  ...
  // MCP tools are namespaced as `<server>__<tool>`. Map known MCP server keys
  // to a friendly brand label so the UI shows e.g. "NevanCode" instead of the
  // raw config key. Unknown servers fall back to capitalized words.
  const MCP_SERVER_DISPLAY_NAMES: Record<string, string> = {
    'nevancode-mcp': 'NevanCode',
  }
  ...
  for (const [serverKey, label] of Object.entries(MCP_SERVER_DISPLAY_NAMES)) {
    if (toolName.startsWith(`${serverKey}__`)) {
      // Slice off only the server key (not the `__`) so capitalizeWords still
      // renders the separator as spaces — matching other MCP tool headers.
      return {
        name: label + capitalizeWords(toolName.slice(serverKey.length)),
        type: 'tool',
      }
    }
  }
