...
/**
 * Freebuff has been removed from this build; the flag stays `false` so dead
 * Freebuff branches across the codebase short-circuit at runtime without
 * rewriting every conditional at once. Typed as `boolean` (not the literal
 * `false`) so existing type narrowing on the truthy branch still compiles.
 */
export const IS_FREEBUFF: boolean = false

/** Retained for type compatibility with any lingering imports. */
...
export const AGENT_MODE_TO_ID = {
  ...
  MAX: 'base2-max',
  PLAN: 'base2-plan',
  'CyberNevan/hunter': 'base2-hunter',
} as const
