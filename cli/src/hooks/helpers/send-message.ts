...
import { maybeAutoCreateGoalFromFirstMessage } from '../../commands/goal'
import { getProjectRoot } from '../../project-files'
import { useChatStore } from '../../state/chat-store'
import { buildGoalContextBlock, useGoalStore } from '../../state/goal-store'
...
import { curateRelevantFilesBlock } from '../../utils/semantic-context'
...
export const prepareUserMessage = async (params: {
  ...
  /** Optional raw user intent; if absent, content is used for the auto-goal. */
  userIntent?: string
  ...
  const { content, agentMode, postUserMessage, attachments, userIntent, deps } =
    params
  const { setMessages, lastMessageMode, setLastMessageMode, scrollToLatest } =
    deps

  // Auto-create a `/goal` from the first user message of every session.
  // Use the raw user intent when provided so command wrappers (/plan, /review,
  // /interview, clear-and-implement seed) don't seed the goal with the
  // prompt-builder boilerplate. Falls back to content for plain messages.
  maybeAutoCreateGoalFromFirstMessage(userIntent ?? content)
  ...
  // Inject the active thread goal as a system context block prepended to
  // finalContent. The agent uses this to stay anchored to the user's goal
  // across every turn. The visible user message is unchanged — only the
  // prompt seen by the agent carries the goal block.
  const activeGoal = useGoalStore.getState().currentGoal
  const goalContextBlock = buildGoalContextBlock(activeGoal)
  if (goalContextBlock) {
    finalContent = finalContent
      ? `${goalContextBlock}\n\n${finalContent}`
      : goalContextBlock
  ...
  // Smart Context Curation (Feature 3): prepend a semantically-curated set of
  // relevant files for this request so the agent starts from the right code
  // instead of rediscovering it turn-by-turn. Best-effort and non-blocking — an
  // empty block (disabled / index not ready / timeout) changes nothing. Only
  // the prompt seen by the agent carries this block; the visible user message
  // is unchanged, mirroring the goal block above.
  try {
    const relevantFilesBlock = await curateRelevantFilesBlock(userIntent ?? content)
    if (relevantFilesBlock) {
      finalContent = finalContent
        ? `${relevantFilesBlock}\n\n${finalContent}`
        : relevantFilesBlock
    }
    ...
    logger.debug(
      ...
      '[send-message] context curation skipped',
    )
  }
...
export const handleRunCompletion = (params: {
  ...
  // Auto-clear the active thread goal only when the agent explicitly signals
  // full completion via the mandatory goal protocol marker. Without this gate,
  // multi-turn tasks would lose their goal after the first successful turn.
  const COMPLETION_MARKER = '🎯 تم حذف الهدف بنجاح وإكمال المهمة بالكامل'
  const outputStr = JSON.stringify(output)
  if (outputStr.includes(COMPLETION_MARKER)) {
    try {
      ...
      // Goal cleanup is best-effort — never block run completion for it.
    }
  }
}
