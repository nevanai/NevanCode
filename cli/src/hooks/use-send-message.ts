...
export const useSendMessage = ({
  ...
  const sendMessage = useCallback<SendMessageFn>(
    async ({ content, agentMode, postUserMessage, attachments, userIntent }) => {
      ...
      let baseAgentDefinitions: AgentDefinition[]
      let baseResolvedAgent: AgentDefinition | string

      try {
        baseAgentDefinitions = loadAgentDefinitions()
        baseResolvedAgent = resolveAgent(
          agentMode,
          agentId,
          baseAgentDefinitions,
        )

        if (
          !IS_FREEBUFF &&
          shouldUseSelectedOpenAiModelForAgent(baseResolvedAgent)
        ) {
          const openAiModelReadiness = await ensureSelectedOpenAiModelReady()
          if (!openAiModelReadiness.ready) {
            setMessages((prev) => [
              ...prev,
              getSystemMessage(openAiModelReadiness.message),
            ])
        ...
        logger.error(
          { error },
          '[send-message] Failed to prepare OpenAI runtime config',
        )
        setMessages((prev) => [
          ...prev,
          createErrorChatMessage(
            '⚠️ Failed to prepare the selected OpenAI model. Please try again.',
          ),
        ])
        await yieldToEventLoop()
        setTimeout(() => scrollToLatest(), 0)
      ...
      try {
        let agentDefinitions = baseAgentDefinitions
        let resolvedAgent = baseResolvedAgent

        if (!IS_FREEBUFF) {
          const runtimeOpenAiConfig = applySelectedOpenAiModelToRuntimeAgents({
            agentDefinitions,
            agent: resolvedAgent,
          })
          agentDefinitions = runtimeOpenAiConfig.agentDefinitions
          resolvedAgent = runtimeOpenAiConfig.agent
        }
