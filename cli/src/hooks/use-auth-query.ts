...
export function useAuthQuery(deps: UseAuthQueryDeps = {}) {
  ...
  // Skip Codebuff backend validation when ChatGPT OAuth is linked — it routes
  // directly to OpenAI. We only check for the presence of credentials (not
  // their momentary validity) so a temporarily-expired access token doesn't
  // re-enable the auth query and surface a spurious "retrying..." indicator;
  // the SDK will auto-refresh the token on the next actual request.
  const isChatGptLinked = !!getChatGptOAuthCredentials()

  // Skip Codebuff backend validation when any BYOK provider key is stored —
  // requests go directly to the provider, no Codebuff token needed.
  const isByokLinked = isAnyByokProviderLinked()

  // Same for a connected Claude Pro/Max account — requests go directly to the
  // native Anthropic API via the OAuth token, so the synthetic Codebuff token
  // must not be validated against the backend.
  const isAnthropicLinked = !!getAnthropicOAuthCredentials()

  // Same for a connected Gemini (Google) account — requests route directly to
  // the Code Assist API via the OAuth token.
  const isGeminiLinked = !!getGeminiOAuthCredentials()
