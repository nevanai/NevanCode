...
export const useConnectionStatus = (
  ...
  useEffect(() => {
    ...
    const checkConnection = async () => {
      // When ChatGPT OAuth credentials exist, we route AI calls directly to
      // OpenAI without the Codebuff backend — always treat as connected so the
      // "connecting..." status never appears for ChatGPT-linked users, even if
      // the access token is temporarily expired (it will be auto-refreshed on
      // the next request via getValidChatGptOAuthCredentials()).
      if (getChatGptOAuthCredentials()) {
        if (isMounted) {
