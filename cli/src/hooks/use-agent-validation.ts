import { getAnthropicOAuthCredentials, getChatGptOAuthCredentials, getGeminiOAuthCredentials, isAnyByokProviderLinked, validateAgents } from '@codebuff/sdk'
...
export const useAgentValidation = (): UseAgentValidationResult => {
  ...
  const validate = useCallback(async (): Promise<ValidationCheckResult> => {
    ...
    try {
      ...
      // When the user is routing through ChatGPT OAuth or a BYOK provider
      // (Fireworks, OpenRouter), the Codebuff backend is not in the request
      // path — hitting /api/agents/validate would add a guaranteed network
      // failure that silently blocks sends. Run validation locally instead.
      const useRemoteValidation =
