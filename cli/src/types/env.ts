...
export type CliEnv = BaseEnv & {
  ...
  NEVAN_SEMANTIC_CONTEXT?: string
  /** Disable auto-injecting curated relevant files into each turn ('0'|'false'|'off'). */
  NEVAN_CONTEXT_INJECTION?: string
  /** Override the curation relevance threshold (0–1) for context selection. */
  NEVAN_CURATION_THRESHOLD?: string

  // GitHub access for `nevan index --org` (Feature 4). Either is accepted.
  GITHUB_TOKEN?: string
  GH_TOKEN?: string
}
