...
export type ChatStoreState = {
  ...
  /**
   * Live context-window usage for the main agent, streamed per-step from the
   * runtime (`context_usage` event). Reflects the REAL model context mid-run —
   * `tokenCount` is the actual token count and `maxTokens` the operational
   * window the pruner compacts against. Null until the first step reports it.
   */
  liveContextUsage: { tokenCount: number; maxTokens: number } | null
...
type ChatStoreActions = {
  ...
  setLiveContextUsage: (
    usage: { tokenCount: number; maxTokens: number } | null,
  ) => void
    ...
    reset: () =>
      set((state) => {
        ...
        state.runState = initialState.runState
          ...
          : null
        state.liveContextUsage = initialState.liveContextUsage
