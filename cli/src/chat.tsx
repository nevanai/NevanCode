...
} from './commands/clear-and-implement'
...
export const Chat = ({
  ...
  const skillsVersion = usePluginStore((state) => state.skillsVersion)
  // Re-derive skills whenever a plugin install/uninstall triggers a reload
  const loadedSkills = useMemo(() => getLoadedSkills(), [skillsVersion])
  ...
  // Hydrate the persisted thread goal once on mount (project root is set
  // before Chat renders). Safe to re-run; the store gates on currentGoal.
  useEffect(() => {
    if (!useGoalStore.getState().currentGoal) {
  ...
  useEffect(() => {
    const handleFollowupClick = (event: Event) => {
      ...
      // Intercept clear-and-implement sentinels coming from a `/plan` response.
      // These prompts never reach the agent — instead we clear the chat
      // context, switch to the chosen mode, and resend the saved plan as the
      // seed message of a fresh implementation session.
      const sentinel = parseClearAndImplementSentinel(prompt)
      if (sentinel) {
        ...
        if (!result.ok) {
          setMessages((prev) => [
            ...prev,
            getSystemMessage(
              result.reason === 'no-plan-found'
                ? 'No plan was found in this conversation to implement. Use /plan first to generate one.'
                : 'Could not start implementation: unknown mode.',
            ),
          ])
        }
        return
  ...
  const handleCommandResult = useCallback(
    (result?: CommandResult) => {
      ...
      if (result.openPluginMode) {
        usePluginStore.getState().openPluginMode()
      }

      if (result.openMcpMode) {
        useMcpStore.getState().openMcpMode()
      }
