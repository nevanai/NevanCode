...
import { PluginDiscoverScreen } from './components/plugin-discover-screen'
...
import { getCliRuntimeIdentity } from './utils/runtime-identity'
...
export const App = ({
  ...
  const runtimeIdentity = useMemo(() => getCliRuntimeIdentity(), [])
  ...
  // Plugin mode state from store
  const { showPluginMode, closePluginMode } = usePluginStore()

  // MCP mode state from store
  const { showMcpMode, closeMcpMode } = useMcpStore()
  ...
  }, [logoComponent, projectRoot, runtimeIdentity, theme])
  ...
  return (
    <AuthedSurface
      ...
      showPluginMode={showPluginMode}
      onCancelPluginMode={closePluginMode}
      showMcpMode={showMcpMode}
      onCancelMcpMode={closeMcpMode}
    />
  )
}
...
interface AuthedSurfaceProps {
  ...
  onCancelPluginMode: () => void
  showMcpMode: boolean
  onCancelMcpMode: () => void
}
...
const AuthedSurface = ({
  ...
  if (showPluginMode) {
    return <PluginDiscoverScreen onCancel={onCancelPluginMode} />
  }

  if (showMcpMode) {
    return <McpScreen onCancel={onCancelMcpMode} />
  }
