...
import { handleGeminiAuthCode } from '../components/gemini-connect-banner'
import { buildInterviewPrompt, buildPlanPrompt, buildReviewPrompt } from './prompt-builders'
import { buildUltrareviewPrompt } from './ultrareview'
...
export async function routeUserPrompt(
  ...
  if (inputMode === 'interview') {
    ...
    sendMessage({
      content: buildInterviewPrompt(trimmed),
      agentMode,
      userIntent: trimmed,
    })
    setTimeout(() => {
  ...
  if (inputMode === 'review') {
    ...
    sendMessage({
      content: buildReviewPrompt('custom', trimmed),
      agentMode,
      userIntent: trimmed,
    })
    setTimeout(() => {
  ...
  // Handle ultrareview mode input (advanced deep review)
  if (inputMode === 'ultraReview') {
    if (!trimmed) return
    saveToHistory(trimmed)
    setInputValue({ text: '', cursorPosition: 0, lastEditDueToNav: false })
    setInputMode('default')
    ...
    sendMessage({
      content: buildUltrareviewPrompt(trimmed),
      agentMode,
      userIntent: trimmed,
    })
    setTimeout(() => {
  ...
  // Handle connect:claude mode input (authorization code, pasted as code#state)
  ...
  // Handle connect:gemini mode input (authorization code pasted from the Google
  // consent / authcode page).
  if (inputMode === 'connect:gemini') {
    ...
    const code = trimmed
    if (code) {
      const result = await handleGeminiAuthCode(code)
      setMessages((prev) => [
        ...prev,
        getUserMessage(trimmed),
        getSystemMessage(result.message),
