...
describe('command-registry', () => {
  ...
  describe('COMMAND_REGISTRY', () => {
    ...
    test('removed commands are not registered or suggested', () => {
      ...
      for (const command of removedCommands) {
        ...
        expect(SLASH_COMMANDS.some((cmd) => cmd.id === command)).toBe(false)
      }

      for (const alias of ['strong', 'sub', 'buy-credits', 'bug', 'report']) {
        expect(findCommand(alias)).toBeUndefined()
        expect(
          SLASH_COMMANDS.some((cmd) => cmd.aliases?.includes(alias)),
        ).toBe(false)
      }
    })
    ...
    test('connect command is available in codebuff', () => {
    ...
    test('connect:chatgpt command resolves in codebuff', () => {
