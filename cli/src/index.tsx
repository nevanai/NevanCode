...
import { initializePluginRegistry } from './utils/plugin-registry'
...
function parseArgs(): ParsedArgs {
  ...
  program
    .name('nevan')
    .description('Nevan CLI - AI-powered coding assistant')
...
async function main(): Promise<void> {
  ...
  // Headless subcommand dispatch (Feature 3 `context`, Feature 4 `index`/`impact`).
  // Handled BEFORE commander parses argv — otherwise their flags (--debug,
  // --repos, --org, …) would be rejected as unknown options — and before the TUI
  // / background indexing boots.
  if (process.argv[2] === 'context') {
    const { runContextCommand } = await import('./commands/context-command')
    const code = await runContextCommand(process.argv.slice(3), {
      cwd: process.cwd(),
  ...
  if (process.argv[2] === 'index') {
    const { runIndexCommand } = await import('./commands/index-command')
    const env = getCliEnv()
  ...
  if (process.argv[2] === 'impact') {
    const { runImpactCommand } = await import('./commands/index-command')
    const code = await runImpactCommand(process.argv.slice(3), {
      cwd: process.cwd(),
      write: (s) => process.stdout.write(s),
      writeErr: (s) => process.stderr.write(s),
    })
  ...
  const isPluginCommand = process.argv[2] === 'plugin'
  ...
  // Initialize plugin registry (loads marketplace cache; background-refreshes if stale)
  await initializePluginRegistry()

  // Handle plugin subcommands (non-interactive)
  if (isPluginCommand) {
    const subcommand = process.argv[3]
    const { install, uninstall, refreshPlugins } = await import('./utils/plugin-registry')

    if (subcommand === 'install') {
      const pluginName = process.argv[4]
      if (!pluginName) {
        console.error('Usage: nevan plugin install <plugin-name>')
        process.exit(1)
      }
      console.log(`Installing ${pluginName}…`)
      const result = await install(pluginName)
      if (result.success) {
        console.log(`Installed ${pluginName} · ${result.skillCount} skill${result.skillCount !== 1 ? 's' : ''}, ${result.mcpCount} MCP server${result.mcpCount !== 1 ? 's' : ''}`)
        process.exit(0)
      } else {
        console.error(`Failed to install ${pluginName}: ${result.error}`)
        process.exit(1)
      }
    } else if (subcommand === 'remove' || subcommand === 'uninstall') {
      const pluginName = process.argv[4]
      if (!pluginName) {
        console.error(`Usage: nevan plugin ${subcommand} <plugin-name>`)
        process.exit(1)
      }
      ...
      if (result.success) {
        ...
        console.error(`Failed to remove ${pluginName}: ${result.error}`)
        process.exit(1)
      }
    } else if (subcommand === 'refresh') {
      console.log('Refreshing plugin marketplace…')
      await refreshPlugins()
      console.log('Marketplace updated')
      process.exit(0)
    } else {
      console.error('Usage: nevan plugin <install|remove|refresh> [plugin-name]')
      process.exit(1)
    }
  }
  ...
  if (isPublishCommand) {
    ...
    if (result.success && result.publisherId && result.agents) {
      logger.info(bold('Successfully published:'))
      ...
      logger.error(bold('Publish failed'))
      if (result.error) logger.error(`Error: ${result.error}`)
      if (result.details) logger.error(result.details)
      if (result.hint) logger.warn(`Hint: ${result.hint}`)
      process.exit(1)
    }
  }
  ...
  const AppWithAsyncAuth = () => {
    ...
    const handleProjectChange = React.useCallback(
      async (newProjectPath: string) => {
        ...
        resetCodebuffClient()
        // Re-target the semantic index at the new project. Background indexing
        // continues; the chat header reflects the new run via useIndexingStore.
        startSemanticContext(newProjectPath).catch(() => {
